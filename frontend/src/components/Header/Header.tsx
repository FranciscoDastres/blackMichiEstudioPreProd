...
function Header() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await api.get<SearchProduct[] | { productos?: SearchProduct[] }>('/productos', {
          params: { q: searchQuery.trim(), limit: 6 },
        });
        const all = Array.isArray(res.data) ? res.data : ((res.data as { productos?: SearchProduct[] })?.productos || []);
