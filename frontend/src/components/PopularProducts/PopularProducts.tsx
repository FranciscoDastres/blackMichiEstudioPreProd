  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await api.get<PopularProduct[]>("/productos");
      ...
      <div className="w-full max-w-[1800px]">
        ...
        <div className="relative">
          <button
            ...
            className="absolute -left-4 sm:-left-6 top-1/2 -translate-y-1/2 z-20 text-foreground/50 hover:text-accent transition-all duration-300 group/arrow"
            onClick={() =>
              (document.querySelector(".popular-products-container") as HTMLElement | null)
          ...
          <div
            className="popular-products-container flex gap-6 pb-6 overflow-x-auto scrollbar-hide px-1"
          ...
          <button
            ...
            className="absolute -right-4 sm:-right-6 top-1/2 -translate-y-1/2 z-20 text-foreground/50 hover:text-accent transition-all duration-300 group/arrow"
            onClick={() =>
              (document.querySelector(".popular-products-container") as HTMLElement | null)
                ?.scrollBy({ left: 400, behavior: "smooth" })
            }
