// frontend/src/components/PaymentReceipt/PaymentReceipt.tsx
...
const PaymentReceipt = () => {
  ...
  useEffect(() => {
    if (pedidoId && token) {
      ...
      const pendingOrder = JSON.parse(localStorage.getItem('pendingOrder') || '{}') as { pedidoId?: string; flowToken?: string };
  ...
  const fetchPedido = async (id: string, flowToken: string) => {
    try {
      const url = `${import.meta.env.VITE_API_URL}/api/payments/pedido/${id}/status?token=${encodeURIComponent(flowToken)}`;
      const response = await fetch(url);
      const data = await response.json() as { success: boolean; pedido: Pedido };
      if (data.success) {
  ...
  const generatePDF = async () => {
    try {
      ...
      const canvas = await html2canvas(element, {
        ...
        onclone: (clonedDoc) => {
          (clonedDoc.querySelector('.receipt-container') as HTMLElement | null)?.style.setProperty('boxShadow', 'none');
          clonedDoc.querySelectorAll('.print-hidden').forEach(el => (el as HTMLElement).style.display = 'none');
        }
      });
