...
import {
    ...
    ArrowRight, PlusCircle, Settings, LucideIcon
} from "lucide-react";
            ...
            cell: (value: unknown) => (
                <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-full bg-sky-500/10 flex items-center justify-center">
                        <span className="text-xs font-medium text-sky-400">#{value as number}</span>
                    </div>
                </div>
            )
        },
            ...
            cell: (_value: unknown, row: Record<string, unknown>) => (
                <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-gray-200">
                        {(row?.comprador_nombre || row?.comprador_email || "Cliente invitado") as string}
                    </span>
                </div>
            )
        },
    ...
    const statCards: StatCard[] = [
        {
            ...
            value: `$${((stats as Record<string, number>).totalSales || 0).toLocaleString("es-CL")}`,
            icon: DollarSign,
            bg: "bg-sky-500/10",
            text: "text-sky-400",
        ...
        {
            label: "Total Pedidos",
            value: (stats as Record<string, number>).orders || 0,
            icon: ShoppingBag,
        ...
        {
            label: "Usuarios",
            value: (stats as Record<string, number>).users || 0,
            icon: Users,
            bg: "bg-purple-500/10",
        ...
        {
            label: "Productos",
            value: (stats as Record<string, number>).products || 0,
            icon: Package,
            bg: "bg-amber-500/10",
            text: "text-amber-400",
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                >
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500">{(stats as Record<string, number>).products || 0} productos activos</p>
                    </div>
            ...
            <div className="bg-gray-900/80 rounded-2xl border border-gray-800 shadow-xl p-6 md:p-8">
                ...
                {lastOrders.length > 0 ? (
                    <div className="overflow-hidden rounded-xl border border-gray-800 bg-gray-900/60">
                        <AdminTable columns={columns} data={lastOrders as unknown as Record<string, unknown>[]} />
                    </div>
                ) : (
