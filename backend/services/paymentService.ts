...
export async function crearPago({
    ...
    direccion,
    cuponCodigo,
}: CrearPagoParams): Promise<CrearPagoResult> {
    ...
    try {
        ...
        let cuponAplicado: cuponService.CuponResumen | null = null;
        let descuento = 0;
        if (cuponCodigo) {
        ...
        const usuarioId: number | null = userResult.rows.length > 0 ? userResult.rows[0].id : null;
...
export async function getPedidoByFlowToken(token: string): Promise<number | null> {
...
export async function procesarWebhook(
    ...
    rawHeaders: unknown,
    ip: string
): Promise<{ pedidoId: number; estado: string }> {
    ...
    const webhookId: number = webhookResult.rows[0].id;
    ...
    const statusMap: Record<number, string> = { 1: 'pendiente', 2: 'pagado', 3: 'rechazado', 4: 'cancelado' };
    const nuevoEstado = statusMap[payment.status] || 'pendiente';

    const client = await pool.connect();
    let pedido: { id: number; comprador_email: string; comprador_nombre: string; total: number; cupon_id: number | null };
    let pedidoCompleto: emailService.Pedido | null = null;
    let itemsRows: emailService.EmailItem[] | null = null;
    try {
        await client.query('BEGIN');
        ...
        const updateResult = await client.query(
            ...
            [
                ...
                parseInt(String(payment.status)),
                nuevoEstado,
