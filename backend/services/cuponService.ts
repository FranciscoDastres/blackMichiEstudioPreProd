...
export interface ValidarCuponResult {
    valido: boolean;
    cupon: CuponResumen | null;
    descuento: number;
    error?: string;
}

export async function validarCupon(codigo: string, totalCarrito: number): Promise<ValidarCuponResult> {
...
export async function incrementarUso(cuponId: number | null | undefined): Promise<void> {
    if (!cuponId) return;
    await db.query(
...
export async function listarCupones(): Promise<unknown[]> {
    const result = await db.query(`SELECT * FROM cupones ORDER BY created_at DESC`);
    return result.rows;
}

export interface CrearCuponData {
    codigo: string;
    descripcion?: string | null;
    tipo: string;
    valor: number | string;
    monto_minimo?: number | string | null;
    usos_maximos?: number | string | null;
...
export async function crearCupon({
    ...
    fecha_expiracion,
    activo,
}: CrearCuponData): Promise<unknown> {
    const result = await db.query(
        `INSERT INTO cupones
...
export async function actualizarCupon(id: number | string, data: Partial<CrearCuponData>): Promise<unknown> {
...
export async function eliminarCupon(id: number | string): Promise<{ success: boolean }> {
