...
export async function emailConfirmacionPago(pedido: Pedido, items: EmailItem[]): Promise<void> {
...
export async function emailNuevoPedidoAdmin(pedido: Pedido, items: EmailItem[]): Promise<void> {
    const itemsHtml = items.map((i) =>
...
export async function emailPedidoEnviado(pedido: Pedido): Promise<void> {
    const trackingUrl = `${FRONTEND_URL}/payment/return?pedidoId=${pedido.id}&token=${encodeURIComponent(pedido.flow_token)}`;
...
export async function emailPedidoEntregado(pedido: Pedido): Promise<void> {
    const trackingUrl = `${FRONTEND_URL}/payment/return?pedidoId=${pedido.id}&token=${encodeURIComponent(pedido.flow_token)}`;
...
export async function emailBienvenida(nombre: string, email: string): Promise<void> {
    await sendEmail({
        to: email,
