...
export interface HeroImages {
    section1: HeroImageData | null;
    section2: HeroImageData | null;
    section3: HeroImageData | null;
    section4: HeroImageData | null;
    section5: HeroImageData | null;
    section6: HeroImageData | null;
}
...
export async function getHeroImages(): Promise<HeroImages> {
    ...
    result.rows.forEach((row) => {
        (images as unknown as Record<string, HeroImageData | null>)[row.section] = {
...
export async function getPublicHeroImages(): Promise<unknown[]> {
    const result = await db.query(
        `SELECT section, image_url, title, subtitle, button_text, categoria
       FROM hero_images
...
export async function getFirstHeroImage(): Promise<HeroImageData | null> {
    const result = await db.query(
...
export async function uploadHeroImage(
    ...
    categoria: string | null
): Promise<HeroImageData & { section: string }> {
...
export async function deleteHeroImage(section: string): Promise<{ success: boolean }> {
    if (!section || !section.match(/^section[1-6]$/)) {
    ...
    const imageUrl: string | undefined = result.rows[0]?.image_url;
